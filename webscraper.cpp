...
int main()
{
  ...
  for (int i = 5; i < req_links.size() - 5; i++)
  {
    ...
    std::cout << "Retreiving text " << i - 4 << "/" << req_links.size() - 9 << std::endl;
  }

  std::cout << "Retrieved all text\n";
  ...
  {
    documents.push_back(
        bsoncxx::builder::stream::document{} << "text" << s << bsoncxx::builder::stream::finalize);
  }
